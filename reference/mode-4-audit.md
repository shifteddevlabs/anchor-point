...
> Promoted 2026-07-07 from the transcript-vault learnings lake (prop_be593b70f8c08675).

- **Confirm you're auditing the real trunk before scoring.** Before Mode 4 (or drafting a "future work" spec), `git fetch` + diff the checked-out branch against `origin/main` (and check merged/open PRs) to rule out a narrow hotfix/feature branch that looks stale while `main` is already healthy. On ai-health-export-app, a doc audit run from `hotfix/v2.1.2-chat-list-hang` scored ~60/100 and was about to re-execute a full v3→v4 doc migration that `main` had already shipped (SESSION-HANDOFF/REFERENCES/handoff-history migration, path fixes, link repairs, and the entire v3.0 Clinical Records feature). Re-baselining against `main` raised the score to ~76 and dropped most findings as already-done. Concurrent/overnight sessions can ship real work between when a branch was cut and when it gets audited.

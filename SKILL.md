...
This `SKILL.md` is the router. The 7 modes each have a procedural reference file under `reference/mode-N-*.md`; load only the one for the mode you're running. Routing tables, core rules, canonical v4 surfaces, naming rules, and the high-frequency output shapes (Review, Inventory, Audit) stay inline here because Modes 1-5 touch them. The audit scoring rubric + doc-placement table and the two ratchet output shapes live in `reference/` (loaded only by Modes 4/6/7) since the session-start Review path never needs them.
...
| `reference/audit-rubric.md` | 100-point audit rubric + hard gates + "Where New Docs Go" placement table (Modes 4/6/7 only) |
| `reference/output-shapes.md` | Project Ratchet + Doc Workflow Ratchet output shapes (Modes 6/7 only) |
...
The 100-point rubric (Hard Gates + category scores + health bands) and the "Where New Docs Go" placement table live in `reference/audit-rubric.md`. Load it for Audit (Mode 4); Mode 6 reuses it for baselines and Mode 7 scores the workflow alongside it. The session-start Review path (Mode 2) never needs the rubric, so it stays out of the hot path.
...
The **Project Ratchet** (Mode 6) and **Doc Workflow Ratchet** (Mode 7) output shapes live in `reference/output-shapes.md`, loaded only when running those two modes.
